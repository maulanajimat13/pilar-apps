            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                        <h4 class="card-title">Edit brand presenter : {{ $presenter->name }}</h4>
                    </div>
                </div>
                <div class="col-lg-5">
                    <div class="card-body">
                        <form action="/dashboard/presenters/{{ $presenter->id }}" method="POST" enctype="multipart/form-data">
                            @method('put')
                            @csrf
                            <div class="mb-3">
                                <label for="name"
                        ...
                        @enderror">BP Name
                                ...
                                <input type="text" class="form-control" id="username" name="username"
                                    value="{{ old('username',$presenter->username) }}" required autofocus>
                                @error('username')
                        ...
                        @enderror">Phone Number
                                ...
                                <input type="text" class="form-control" id="phone_number" name="phone_number"
                                    value="{{ old('phone_number', $presenter->phone_number) }}">
                                @error('phone_number')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                        ...
                        @enderror">Qris Name
                                ...
                                <input type="text" class="form-control" id="qris_name" name="qris_name"
                                    value="{{ old('qris_name', $presenter->qris_name) }}">
                                @error('qris_name')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="form-group">
                                ...
                                @enderror">Qris Image</label><br>
                                <input type="hidden" name="oldImage" value="{{ $presenter->image }}">
                                @if ($presenter->image)
                                <img class="img-preview img-fluid mb-3 col-sm-5" src="{{ asset('storage/' . $presenter->image) }}">
                                 @else
