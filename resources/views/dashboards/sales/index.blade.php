          ...
          <div class="card">
             <div class="card-header d-flex justify-content-between">
                <div class="header-title">
                   ...
                   <a href="/dashboard/sales/create" class="btn btn-primary my-3">Input Sales</a>
                   <a href="/dashboard/sales/export_excel" class="btn btn-success my-3">Export</a>
                </div>
             </div>
             <div class="card-body">
                   ...
                   <table id="datatable" class="table table-striped" data-toggle="data-table">
                      ...
                      <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $sale->team->name }}</td>
                            <td>{{ $sale->presenter->name }}</td>
                            <td>{{ $sale->cc }}</td>
                            <td>{{ $sale->ecc }}</td>
                            <td>{{ $sale->gender }}</td>
                            <td>{{ $sale->age }}</td>
                            <td>{{ $sale->rokok_asal }}</td>
                            <td>{{ $sale->other_info}}</td>
                            <td>{{ $sale->clasmild }}</td>
                            <td>{{ $sale->redmax }}</td>
                            <td>{{ $sale->silver}}</td>
                            <td>
                                ...
                                {{-- <a href="/dashboard/sales/{{ $sale->id }}/edit" class="badge bg-warning">
                                ...
                                <form action="/dashboard/sales/{{ $sale->id }}" method="post" class="d-inline">
                                    @method('delete')
                                    @csrf
                                    <button class="badge bg-danger border-0" onclick="return confirm('Are you sure  to delete sales with id : {{ $sale->id }} ?')">
